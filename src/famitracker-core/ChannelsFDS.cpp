...
CChannelHandlerFDS::CChannelHandlerFDS(SoundGen *gen) : CChannelHandler(gen)
{ 
	SetMaxPeriod(0xFFF);
...
{
	// Check wave changes
	// TODO - dan: HasWaveChanged
	if (m_iInstrument != MAX_INSTRUMENTS && false/*&& theApp.GetSoundGenerator()->HasWaveChanged()*/)
	{
